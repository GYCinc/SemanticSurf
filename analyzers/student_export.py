...
class StudentExporter:
    ...
    def export_markdown(self) -> str:
        ...
        # --- NEW: LeMUR (LLM) Analysis ---
        if self.analysis and 'lemur_analysis' in self.analysis:
            lemur = self.analysis['lemur_analysis']
            md.append("## 🤖 Deep AI Analysis (Paid)")
            md.append(f"**Your Prompt:** _{lemur.get('prompt', 'N/A')}_")
            md.append("")

            # This 'response' key is what we saved in lemur_query.py
            if 'response' in lemur:
                # LeMUR often returns markdown, so we just append it raw
                md.append(lemur['response'])
        ...
        md.append("")
        md.append("---")
        md.append("*Generated by Semantic Surfer*")
