...
## 5. Hub Authority & Sanity Sync
- **The Hub API:** Central endpoint for raw session ingestion (`/api/mcp`).
- **The Petty Dantic Gateway:** `/api/admin/sessions/curate` is the sovereign bridge for Publish/Delete actions.
- **Sanity-First Delivery:** All Post-Lesson Cards (Inbox Items) are created in **Sanity**. This is the absolute source of truth for student visibility.
- **Human Authority:** The Manager's approval via the Gateway is required before data enters the permanent PCL or Sanity.
